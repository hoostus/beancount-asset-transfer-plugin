...
def transfer(entries, option_map):
    ...
    used_lots = collections.defaultdict(lambda : collections.defaultdict(Decimal))
    for entry in entries:
        if isinstance(entry, data.Custom) and entry.type == CUSTOM_DIRECTIVE:
            ...
            saved_amount = Amount(amount.value.number,amount.value.currency)
            ...
            postings = []
            #print(entry.date,source_account," => ",destination_account," ",amount)
            # Process lots LIFO
            lots = sorted(accounts[source_account], key=lambda a: a.acquisition_date, reverse=True)
            for idx,lot in enumerate(lots):
                if lot.acquisition_date > entry.date:
                    continue
                ...
                amt_used = used_lots[source_account][idx]
                if currency_units.number - amt_used <= 0:
                    #print("THIS LOT ALL USED UP",idx,currency_units.number,amt_used)
                    continue

                reduce_amount = min(currency_units.number - amt_used, amount.number)
                found_amount += reduce_amount
                used_lots[source_account][idx] += reduce_amount
                ...
                #print(entry.date,"TRANSFER ",source_account,amount,idx,cost)

                if amount.number == 0:
                    new_entry = data.Transaction(flag = beancount.core.flags.FLAG_TRANSFER,
                        payee = None,
                        narration = 'Automatically generated by asset transfer plugin {} {}'.format(saved_amount.number,saved_amount.currency),
                        date = entry.date,
